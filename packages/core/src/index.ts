...
export function readProjectConfig(projectRoot: string): ProjectConfig {
  ...
  const parsed: unknown = JSON.parse(raw);
  if (!isRecord(parsed)) {
    throw new Error("Invalid contract-kit.config.json: expected an object");
  }

  const schemaVersion = parsed.schemaVersion;
  if (schemaVersion !== 1) {
    throw new Error("Invalid contract-kit.config.json: schemaVersion must be 1");
  }
  ...
  if (typeof contractsDir !== "string" || contractsDir.trim().length === 0) {
    throw new Error("Invalid contract-kit.config.json: contractsDir must be a non-empty string");
  }
  if (typeof outputDir !== "string" || outputDir.trim().length === 0) {
    throw new Error("Invalid contract-kit.config.json: outputDir must be a non-empty string");
  }
